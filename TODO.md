...
## T2 · 组件样式：Card / Button / NumberStepper / Navbar  「进行中」
...
### 验收标准
- [ ] `arrangeSeats(n)` 返回长度为 8 的数组，有人座位存排名数字（1-based），空位存 `null`
- [ ] 单数 n（1/3/5/7）使用中心向左填位顺序 `[4,5,3,6,2,7,1,0]`，取前 n 个
- [ ] 偶数 n（2/4/6/8）使用中心对填位顺序 `[3,4,2,5,1,6,0,7]`，取前 n 个
- [ ] 双方均为偶数时，主方不翻转；其余情况（奇-奇、奇-偶、偶-奇）主方结果做 `reverse()`
- [ ] 对坐验证（console 输出或手动检查）：A 索引 i ↔ B 索引 7−i，非偶数×偶数时 1 号索引之和 = 7
- [ ] n=0 返回全 null 数组
- [ ] n=8 返回全满数组（无 null）
...
| **修改目标** | 实现"下载 SVG"按钮功能，导出当前平面图为 .svg 文件 |
| **允许修改范围** | 修改 `index.html` 的 `<script>` 块，新增 `downloadSVG()` 函数 |
| **参考来源** | ARCHITECTURE.md §1（导出方案：Blob + `<a download>`） |

### 验收标准
- [ ] 点击"下载 SVG"按钮触发浏览器下载
- [ ] 下载文件名为 `会议座次_客X_主Y.svg`（X/Y 为当前人数）
- [ ] SVG 文件包含完整的平面图（桌子、座位、数字、标签、白板、门），与预览区一致
- [ ] SVG 文件可在浏览器中直接打开查看
- [ ] SVG 文件可拖入 PowerPoint / Word / Illustrator 并正常显示、可编辑
- [ ] SVG 根元素包含 `xmlns="http://www.w3.org/2000/svg"`

---

## T10 · 打印样式 @media print
...
| **参考来源** | DESIGN.md §9（打印样式规范） |

### 验收标准
- [ ] 打印预览中导航栏隐藏
- [ ] 打印预览中左侧输入区、座次规则卡片隐藏
- [ ] 打印预览中右侧操作按钮（下载/打印）隐藏
- [ ] 打印预览中仅显示：平面图 SVG + 图例 + 使用提示
- [ ] SVG 自动缩放适配 A4 宽度
- [ ] 打印预览中 SVG 颜色正常（非屏幕专用色）
...
| **预计耗时** | 20 分钟 |
| **依赖** | T1–T10 全部完成 |
| **修改目标** | 运行 5 组预设案例，逐项核对 ARCHITECTURE.md §9 的全部验收标准 |
| **允许修改范围** | 仅修正 bug，不新增功能 |

### 验收标准
- [ ] **案例 1：客 5·主 5（奇-奇）** → 1 号对坐成立（客 1 号索引 4 + 主 1 号索引 3 = 7 ✓），空位方块数正确，主方已翻转
- [ ] **案例 2：客 3·主 3（奇-奇）** → 1 号对坐成立，各有 5 个空位
- [ ] **案例 3：客 8·主 8（偶-偶）** → 双方均有 8 个实心方块、无空位，主方未翻转，1/2 号各自居中
- [ ] **案例 4：客 4·主 6（偶-偶）** → 主方未翻转，双方 1/2 号居中
- [ ] **案例 5：客 0·主 2** → 长边 A 无座位方块仅标签，长边 B 显示 2 个实心方块
- [ ] 所有案例 SVG 下载后可在 PPT 中打开编辑
- [ ] 所有案例 Ctrl+P 打印预览正常
- [ ] 文件在无网络环境下双击可正常使用
